...
figure; 
subplot(221); im(x,y,fliplr(meanFBloch_spec)/sin(nom_fa*pi/180),[0 1]); colormap(gca,'gray');
...
subplot(222); im(x,y,fliplr(meanFBloch_specspat)/sin(nom_fa*pi/180),'cbar',[0 1]); colormap(gca,'gray');
...
subplot(223); im(x,y,fliplr(angle(meanFBloch_spec)*180/pi),[-180 180]); colormap(gca,'hsv');
...
subplot(224); im(x,y,fliplr(angle(meanFBloch_specspat)*180/pi),'cbar',[-180 180]); colormap(gca,'hsv');
...
% hard pulse
subplot(231); im(x,y,fliplr(sNRMSE_space_hard),[0 1]); colormap(gca,'default'); 
...
subplot(234); im(x,y,fliplr(phsRMSE_space_hard),[0 90]); colormap(gca,'default'); 
...
subplot(232); im(x,y,fliplr(sNRMSE_space_spec),[0 1]); colormap(gca,'default');
set(gca,'Position',[0.3750	0.5450 0.2150	0.3350])
xticks([-12 0 11.9]); xticklabels({'-12','0','12'}); 
yticks([-12 0 11.9]); yticklabels({'-12','0','12'});
title(sprintf('Purely Spectral Pulse \n E. NRMSE=%3.2f',sNRMSE_spec));
subplot(235); im(x,y,fliplr(phsRMSE_space_spec),[0 90]); colormap(gca,'default');
set(gca,'Position',[0.3750	0.0950 0.2150	0.3350])
xticks([-12 0 11.9]); xticklabels({'-12','0','12'}); 
yticks([-12 0 11.9]); yticklabels({'-12','0','12'}); 
title(sprintf('Purely Spectral Pulse \n Phase RMSE=%3.1f',phsRMSE_spec));
% spectral-spatial pulse
subplot(233); im(x,y,fliplr(sNRMSE_space_specspat),[0 1]); h=colorbar; colormap(gca,'default');
set(gca,'Position',[0.6500	0.5450 0.2150	0.3350])
xticks([-12 0 11.9]); xticklabels({'-12','0','12'}); 
yticks([-12 0 11.9]); yticklabels({'-12','0','12'}); 
set(h,'YTick',[0 0.5 1]);
title(sprintf('Spectral-Spatial Pulse \n E. NRMSE=%3.2f',sNRMSE_specspat));
subplot(236); im(x,y,fliplr(phsRMSE_space_specspat),[0 90]); h=colorbar; colormap(gca,'default');
